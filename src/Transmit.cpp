...
void transmitter()
{
  ...
  // Node anchor send data report for host anchor (comprised sync and location packet)
  msgToa_t report = {0};
  report.txId = idFrom; // No matching between String and uint64_t
  report.rxId = TXID_ANCHOR;
  report.rxTs = rxTimestamp;
  report.seqNr = sentNum;

  //////// Need to transmit struct msgToa_t instead of string msg (setData, getData func)
